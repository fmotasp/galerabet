...
const AccessRow: React.FC<{ access: Access; preloadedDriveFiles?: DriveFileItem[]; forceExpand?: boolean; onEdit: (a: Access) => void; onDelete: (id: string) => void }> = ({ access, preloadedDriveFiles, forceExpand, onEdit, onDelete }) => {
  ...
  useEffect(() => {
    if (forceExpand && isDriveLink) {
      setIsExpanded(true);
    } else if (!forceExpand) {
      setIsExpanded(false);
    }
  }, [forceExpand, isDriveLink]);
            ...
            <tbody>
              {isLoadingAccesses ? (
                ...
                filteredAccesses.map((acc) => {
                  const q = searchQuery.toLowerCase();
                  let filteredDriveFiles = driveIndex[acc.id];
                  
                  if (q && filteredDriveFiles) {
                    const providerMatches = acc.title.toLowerCase().includes(q) || acc.login.toLowerCase().includes(q);
                    // If the provider itself doesn't match the search, filter the files!
                    if (!providerMatches) {
                      filteredDriveFiles = filteredDriveFiles.filter(f => f.name.toLowerCase().includes(q));
                    }
                  }
                  ...
                  return (
                    <AccessRow 
                      key={acc.id} 
                      access={acc} 
                      preloadedDriveFiles={filteredDriveFiles} 
                      forceExpand={!!q}
                      onEdit={handleEdit} 
