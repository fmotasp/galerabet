...
export const AppProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
    ...
    if (saved) {
      try {
        ...
        if (
          ...
          // Garante que 'in_review' / 'Em Revisão' seja filtrado
    ...
    const loadInitialSupabaseData = async () => {
      try {
        ...
        if (projErr) {
          ...
          // Only delete google-drive-token if it is not valid
          // (Preserve google-drive-token for shared team access)
  ...
  // Auto-move overdue tasks (2+ days delayed in backlog, nova demanda/pedidos, revisão or active progress) to 'overdue' (Atrasadas / Urgente)
  useEffect(() => {
    if (!tasks || tasks.length === 0) return;

    const overdueCandidateTasks = tasks.filter((t) => {
      // Must not be already finished / completed
      if (isTaskCompleted(t)) return false;
      // If already in overdue status, no need to move
      if (t.status === 'overdue') return false;

      // Check if task has 2 or more days overdue
