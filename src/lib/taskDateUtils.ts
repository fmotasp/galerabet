 ...
 * As per rule: a task only counts as overdue when it has at least 2 days of delay
 * from the PRAZO PREVISTO (dueDate). For example, if due on Sep 8, it only counts as overdue
 * on Sep 10 or later (diffDays >= 2).
 * If the task is not overdue (< 2 days), is completed, or has a future/today due date, returns 0.
 */
 ...
 * Requires at least 2 days of delay (e.g. prazo 8.set -> overdue on 10.set).
 */
