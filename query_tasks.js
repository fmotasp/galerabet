...
async function checkDb() {
  console.log("Fetching recent activity of tasks...");
  
  // Get all tasks sorted by last_moved_at
  ...
  for (const task of data) {
    ...
    console.log(`Last Moved: ${new Date(task.last_moved_at).toISOString()}`);
    if (task.activity_log) {
      console.log("Recent activity:");
      const recent = task.activity_log.slice(-3);
      recent.forEach(act => {
